...
folder = 'uncorrected_varadapt_all';

for ii = 1:length(participant{1})
    datadir = fullfile(data_path,participant{1}{ii});
    cd(datadir);
    output_folder = fullfile('/data/pt_02199/maps_filtered_navs',participant{1}{ii},folder,'pmcON');
    ...
    cd(fullfile(datadir, folder,'pdw_pmcON_kp_mtflash3d_v1c_mpmnav'))
    PDw_nii = dir('*Mag*');
    kk=1;
    while kk<(length(PDw_nii)+1)
           PDw{kk,1} = (fullfile(datadir, folder,'pdw_pmcON_kp_mtflash3d_v1c_mpmnav', PDw_nii(kk).name));
           kk=kk+1;
    end
    % identify t1w magnitude images
    cd(fullfile(datadir, folder,'t1w_pmcON_kp_mtflash3d_v1c_mpmnav'))
    T1w_nii = dir('*Mag*');
    kk=1;
    while kk<(length(T1w_nii)+1)
           T1w{kk,1} = (fullfile(datadir, folder,'t1w_pmcON_kp_mtflash3d_v1c_mpmnav', T1w_nii(kk).name));
           kk=kk+1;
    end
