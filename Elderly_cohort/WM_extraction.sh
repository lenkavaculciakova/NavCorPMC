...
# select 
declare -a corrections=("uncorrected/pdw_01/" "f0-corrected_varadapt_virt/pdw_01/") 
...
do
    ...
    cd $folder
    for dirs in ls -d -- c220*/;
    do
        WMmask_folder=$dirs
    done
    WMmask=$folder$WMmask_folder${WMmask_folder%/}"_gnl_cp_replace_val_replace_val_er2.nii"
    ...
    do
        ...
        do
            matlab -nodesktop -nodisplay -r "cd('/data/pt_02133/1-scripts/navigators'); brain_extraction_job('$path${map%.nii}/${map%.nii}_gnl_cp.nii', '$WMmask', '_WM'); exit"
        done
        ...
        for map in 20*PD.nii 20*R1.nii 20*R2s_WOLS.nii;
        do
            matlab -nodesktop -nodisplay -r "cd('/data/pt_02133/1-scripts/navigators'); brain_extraction_job('$path${map%.nii}/spm_r${map%.nii}_gnl_cp.nii', '$WMmask', '_WM'); exit"
        done
